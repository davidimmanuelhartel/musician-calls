...
async function signIn(page: Page, email: string, next = '/en/dashboard') {
  let messageId = '';
  await expect
    .poll(async () => {
...
test('mobile: bilingual draft, PDF, verified publish, anonymous response, selection', async ({
  ...
  await expect(page.getByLabel('Ensemblets navn')).toHaveValue(`Test Orchestra ${suffix}`);
  await expect(page.getByText('Beethoven5.pdf', { exact: false }).first()).toBeVisible();
  await page.getByRole('link', { name: 'EN', exact: true }).click();
  await page.getByRole('button', { name: 'Verify email to publish' }).click();
  await expect(page.getByText('Check your inbox')).toBeVisible();
  await signIn(page, organizer, '/en/calls/new');
  await expect(page.getByLabel('Ensemble name')).toHaveValue(`Test Orchestra ${suffix}`);
  await expect(page.getByText('Beethoven5.pdf', { exact: false }).first()).toBeVisible();
  ...
  await expect(page).toHaveURL(/\/en\/calls\/new$/);
  await expect(page.getByText('Beethoven5.pdf', { exact: false }).first()).toBeVisible();
